...
class QANerProcessor(DataProcessor):
    def get_train_examples(self, data_dir,info="question"):
        ...
        return self._create_examples(
            self._read_jsonlines(os.path.join(data_dir, "train.jsonl")), "train",info)

    def get_dev_examples(self, data_dir,info="question"):
        """See base class."""
        return self._create_examples(
            self._read_jsonlines(os.path.join(data_dir, "val.jsonl")), "dev",info)

    def get_test_examples(self, data_dir,info="question"):
        """See base class."""
        return self._create_examples(
            self._read_jsonlines(os.path.join(data_dir, "test.jsonl")), "test",info)

    def get_labels(self):
        return ["B-ANS","I-ANS","O"]

    def _create_examples(self,lines,set_type,info="question"):
        examples = []
        for i,json in tqdm(enumerate(lines),"Reading JsonFile"):
            guid = json["qid"]
            txinfo = ""
            if info=='all':
                exmpls = [tup[0] for tup in json['example']]
                extxt = ' '.join(exmpls)
                txinfo = json["question"]+" : "+json["indiv_definition"]+" : "+ extxt
            elif info == 'what_type_q':
                txinfo = "what "+json['entity_type']+" ? "
            elif info == "what_type":
                txinfo = "what "+json['entity_type']
            elif info == "what":
                txinfo = "what "
            ...
            if "definiton" in info:
                txinfo = json["entity_type"]+" : "+ json[info]
                
            if info == "example" :
                exmpls = [tup[0] for tup in txinfo]
                extxt = ' '.join(exmpls)
                txinfo = json["entity_type"]+" : "+ extxt
                
            words = txinfo + " :: " + json["text"]
            label = generate_bio(words,json["answer"])
            words = words.split(" ")
            answer_len = len(json["answer"])
#             counts = answer_len if answer_len < 11 else 10
            counts = 1 if answer_len > 0 else 0
            
            examples.append(InputExample(guid=guid,words=words,labels=label,counts=counts,orig_row=json))
        return examples
    

def read_examples_from_file_qa(data_dir,mode,info="question"):
